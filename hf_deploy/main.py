...
async def lifespan(app: FastAPI):
    ...
    t0 = time.time()
    models["cnn"]         = keras.saving.load_model(str(BASE_DIR / "backend_model.keras"), compile=False)
    print(f"    [+] CNN classifier   ({time.time()-t0:.1f}s)")
    models["autoencoder"] = keras.saving.load_model(str(BASE_DIR / "autoencoder_model.keras"), compile=False)
    print(f"    [+] Autoencoder      ({time.time()-t0:.1f}s)")
    models["unet"]        = keras.saving.load_model(str(BASE_DIR / "unet_model.keras"), compile=False)
...
async def enhance(file: UploadFile = File(...)):
    ...
    if elapsed_ms > 400:  # tightened target: 400ms
        print(f"[/api/enhance] WARNING: {elapsed_ms} ms exceeds 400 ms target")
...
async def analyze(file: UploadFile = File(...)):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # STAGE 03 — Enhancement: premium CLAHE→Wavelet→TDR chain only
    # _compute_multi_enhance removed (duplicate of premium_chain — saved ~200ms)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # STAGE 06 — Detection / Anomaly  (reuses batch)
    t0 = time.perf_counter()
    det = stage_06_detection(arr)   # arr already 128×128; batch built inside
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # STAGE 07 — Segmentation  (reuses batch)
    t0 = time.perf_counter()
    ...
    enhancement = {
        ...
        "multi": None,  # removed — duplicate of premium_chain (saved ~200ms)
